...
//studentService = (StudentService) rmiProxyFactoryStudent2.getObject();
    ...
    public ProfessionService getProfessionService() {
        ...
        if (1 == randomNum) {
            try {
                professionService = (ProfessionService) Naming.lookup("rmi://localhost:1199/jobService");
            } catch (Exception e) {
                logger.info("服务器1挂了，{}" + e);
                try {
                    professionService = (ProfessionService) Naming.lookup("rmi://localhost:1200/jobService");
                } catch (Exception e1) {
                    logger.info("服务器1,2都挂了，{}" + e1);
                }
            }
            ...
            try {
                professionService = (ProfessionService) Naming.lookup("rmi://localhost:1200/ProfessionService");
            } catch (Exception e) {
                logger.info("服务器2挂了，{}" + e);
                try {
                    professionService = (ProfessionService) Naming.lookup("rmi://localhost:1199/ProfessionService");
                } catch (Exception e1) {
                    logger.info("服务器1,2都挂了，{}" + e1);
                }
            }
